...
class SI {
    ...
    public:
        SI (string indexFileName_value, string invertedListFileName_value) : indexFileName (indexFileName_value), invertedList (invertedListFileName_value) {
            fstream indexFile (indexFileName, ios::in | ios::out);
            if (indexFile.fail())
            {
        ...
        void reflectOnDelete (string sk, string pk) {
            ...
            if (RRN_of_SK_of_deletedRecord == -1 || si[RRN_of_SK_of_deletedRecord].refPtr == -1)
            {
                cout << "no record with this secondary key exists already";
            }
            else
            {
                pair<int, int> prev_and_newRefPtr = invertedList.reflectOnDelete(si[RRN_of_SK_of_deletedRecord].refPtr, pk);
                if (prev_and_newRefPtr.first == -1)
                {
                    si[RRN_of_SK_of_deletedRecord].refPtr = si[prev_and_newRefPtr.second].refPtr;
                }

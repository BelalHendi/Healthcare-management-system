...
class SI_IL {
    ...
    public:
        SI_IL (string invertedListFileName_value) : invertedListFileName (invertedListFileName_value) {
            fstream invertedListFile (invertedListFileName, ios::in | ios::out);
            ...
            else
            {
                ...
                else
                {
                    ...
                    while (!invertedListFile.eof())
                    {
                        ...
                        invertedListFile.seekg(1, ios::cur);
                        invertedListFile.read((char*) &(rec.availList_refPtr), sizeof(rec.availList_refPtr));
                        invertedListFile.seekg(1, ios::cur);
                        il.push_back(rec);
                    }
                }            
        ...
        ~SI_IL () {
            ...
            for (int i = 0; i < il.size(); i++)
            {
                ...
                invertedListFile.write((char *) '|', 1);
                invertedListFile.write((char *) &il[i].availList_refPtr, sizeof(il[i].availList_refPtr));
        ...
        pair <int, int> reflectOnDelete (int RRN, string pk) {
            ...
            {
                il[RNN_before_and_containing.first].refPtr = il[RNN_before_and_containing.second].refPtr;
            }
